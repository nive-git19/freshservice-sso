...
app.post('/login', (req, res) => {
  ...
  if (portal && agent.workspace !== portal)
    return res.status(403).json({ success: false, error: `Access denied. You are not an agent for the ${portal} portal.` });

  const now = Math.floor(Date.now() / 1000);
  const payload = { sub: agentKey, email: agentKey, iat: now, exp: now + 300, jti: uuidv4() };
